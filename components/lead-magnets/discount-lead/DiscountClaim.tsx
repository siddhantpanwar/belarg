...
export default function DiscountClaim({ campaignSource = "default" }: DiscountClaimProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if this email already had a discount code
      if (data.isExisting) {
        // Show toast or error message
        setError("One person can only get one discount code. We've sent your existing code to your email again.");
        
        // You can still track the lead event
        if (typeof window !== "undefined" && window.fbq) {
          window.fbq("track", "Lead");
        }
        
        // Optionally redirect to thank you page with the existing code
        router.push(
          `luxury-discount/thank-you?code=${data.discountCode}&name=${encodeURIComponent(name)}&campaign=${campaignSource}`
        );
        return;
      }
      ...
      router.push(
        `luxury-discount/thank-you?code=${discountCode}&name=${encodeURIComponent(name)}&campaign=${campaignSource}`
      );
    } catch (err) {
